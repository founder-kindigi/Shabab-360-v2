/**
 * Tests for:
 *   GET  /api/admin/collaboration-teams/[teamId]/members
 *   POST /api/admin/collaboration-teams/[teamId]/members
 *
 * Covers: capability gate, 404 team/staff, city-scope allow/deny,
 * cross-city staff denial, duplicate active-membership 409,
 * malformed JSON 400, schema validation 400, pagination, status filter.
 */
...
const url = (teamId: string) =>
  `http://localhost/api/admin/collaboration-teams/${teamId}/members`;
...
describe("GET /api/admin/collaboration-teams/[teamId]/members", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(db.staffTeamMembership.findMany).mockResolvedValue([] as any);
    vi.mocked(db.staffTeamMembership.count).mockResolvedValue(0);
  });
