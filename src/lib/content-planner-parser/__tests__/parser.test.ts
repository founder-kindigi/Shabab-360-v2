...
describe("parseSheet", () => {
  ...
  it("off day marker → isOffDay true", () => {
    const { sheet } = parseSheet("AP", [rd], ctx);
    expect(sheet!.sessions[0].isOffDay).toBe(true);
  });

  it("cancelled/no session → isOffDay true", () => {
...
describe("computeSummary", () => {
  it("multiple sessions", () => {
    ...
    expect(s.totalBlocks).toBe(6);
    expect(s.blocksPerTeam.sports).toBe(4);
