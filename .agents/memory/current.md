...
- Confirmed role boundary: City Head has no Cities access and may manage only
  ...
  - Attendance & Park Operations Flow Redesign (`release/attendance-flow`): Branch created off `release/p0-real-data-stabilized`. Expanded Attendance Hub into a full 5-Tab Workspace matching `Shabab_Batch_4_Attendance (1).xlsx`: 1) **Park Roster & Marking Grid** (Halqa Groups: Group 1/2/3 & Staff, Age, Class/Grade badges, 10-record pagination, WhatsApp absentee opener), 2) **Park Comparison Leaderboard & Educational Grade Stats** (Hafiz, 9th/10th, 1st/2nd Year, University), 3) **Monthly Trend Progression** (May 2026 – Apr 2027), 4) **Murabbi & Staff Mashawara Log** (30 session meetings), 5) **Global OFF Weekends & Holiday Calendar**. Includes Excel Import API (`/api/admin/attendance/import`). Verified with 45/45 Vitest tests passing and 0 TypeScript errors.
