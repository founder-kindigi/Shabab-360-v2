...
- Confirmed role boundary: City Head has no Cities access and may manage only
  ...
  - Attendance & Park Operations Flow Redesign (`release/attendance-flow`): Branch created off `release/p0-real-data-stabilized`. Implemented exact 2-tier attendance workflow architecture: 1) **Dedicated Murabbi Attendance Desk** (completely separate workflow, restricted access marked exclusively by Park Lead or delegated staff via `Delegate Access` modal), 2) **Shabab Student Attendance Desk** (hierarchical pipeline: Select Park ➔ Select Halqa Group ➔ Student Marking Grid, markable by Park Lead, Park Admin, or Murabbi). Includes 10-record pagination and WhatsApp absentee opener. Verified with 45/45 Vitest tests passing and 0 TypeScript errors.
