...
- On 2026-09-08, enforced single active batch per city business rule and removed universal tester access in PWA:
  ...
  3. `src/components/modules/admin/mobile-more-page.tsx`: Strict role gating restored across all desks, access tools, and settings. Students, Guardians, and Murabbis only access their authorized tools.
  4. `src/lib/auth/authorize.ts`: Reverted `isHqAdmin` bypass in `requireRole` to restore strict role checking.
  5. Multi-role boundary test suite (75/75 tests), batch route tests (29/29 tests), and TypeScript (`tsc --noEmit`, 0 errors) verified.
  6. Successfully deployed to Vercel production (`https://shabab360.vercel.app`), verified live HTTP 200 OK. Commit `aca9b06`.
- On 2026-09-08, standard organizational terminology updated across the entire application on branch `v2`: "Cadet" replaced with "Shabab" and "Halqa" replaced with "Group" across all UI headers, navigation, dashboard stat pills, roster previews, badges, forms, placeholders, and notification feeds. Underlying database models (`Participant`, `Group`) and import alias flexibility preserved. TypeScript: 0 errors (`tsc --noEmit`). Tests: 183/183 pass across 10 suites (`src/__tests__/release/`, `src/lib/notifications/`, `src/__tests__/api/student-profile/`).
